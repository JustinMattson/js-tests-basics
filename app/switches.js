...
function daysPosition(day, offset) {
  let indexOfWeekDay = -1;
  var err = "oops";
  var dayOfWeek = -1;

  //dow = day of week
  const dowFalse = [1, 2, 3, 4, 5, 6, 7];
  const dowTrue = [7, 1, 2, 3, 4, 5, 6];
  const dowString = [
    "Sunday",
    "Monday",
    "Tuesday",
  ...
  indexOfWeekDay = dowString.findindex(day);
  if ((indexOfWeekDay = -1)) {
    err = "That's not a day of the week";
    return err;
  } else {
    switch (offset) {
      case true:
        dayOfWeek = dowTrue[indexOfWeekDay];
        break;
      case false:
        dayOfWeek = dowFalse[indexOfWeekDay];
      default:
        err = "That's not a valid offset";
        break;
    }
    return `${dayOfWeek}`;
  }
